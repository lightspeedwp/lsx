...
;(function($, window, document, undefined) {
	...
	lsx.set_main_menu_as_fixed = function() {
		if ($('body').hasClass('top-menu-fixed')) {
			$('header.banner.navbar-static-top').scrollToFixed({
				marginTop: function() {
	...
	lsx.set_search_form_effect_mobile = function() {
		$document.on('click', 'header.banner #searchform button.search-submit', function(e) {
			if (windowWidth < 1200) {
				e.preventDefault();
		...
		$document.on('blur', 'header.banner #searchform .search-field', function(e) {
			if (windowWidth < 1200) {
				var form = $(this).closest('form');
	...
	lsx.set_comments_anchor_effect = function() {
		...
		if (document.location.hash == '#comments') {
			margin = $('body').hasClass('top-menu-fixed') ? $('header.banner').height() : 0;
